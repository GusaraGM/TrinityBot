...
def get_class(update: Update, context: CallbackContext) -> int:
    ...
    if user:
        ...
        c.execute("INSERT INTO users (user_id, name, email, class, faculty) VALUES (?, ?, ?, ?, ?)",
                  (user_id, context.user_data['name'], context.user_data['email'], context.user_data['class'], context.user_data['faculty']))
        conn.commit()
        conn.close()
        update.message.reply_text("Регистрация успешно завершена!")
        update.message.reply_text(FACULTY_LINKS[query.data])  # Отправляем ссылку
    return ConversationHandler.END
...
def list_users(update: Update, context: CallbackContext) -> None:
    ...
    if not faculty:
        update.message.reply_text("Сначала выберите факультет командой /start.")
        return

    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("SELECT name, class FROM users WHERE faculty = ?", (faculty,))
    users = c.fetchall()
    conn.close()

    if users:
        user_list = "\n".join([f"{name} - класс {class_}" for name, class_ in users])
        update.message.reply_text(f"Пользователи на факультете {faculty}:\n{user_list}")
    else:
        update.message.reply_text(f"На факультете {faculty} пока нет зарегистрированных пользователей.")

def main() -> None:
    # Make sure to replace 'YOUR_BOT_API_TOKEN' with your actual bot token
    updater = Updater("6116656907:AAGQoOaP7aAIbnHOGp8RGjuK07w4BI-5a18", use_context=True)
    dispatcher = updater.dispatcher
